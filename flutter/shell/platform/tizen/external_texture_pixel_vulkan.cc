...
ExternalTexturePixelVulkan::ExternalTexturePixelVulkan(
  ...
  FT_LOG(Error) << "ExternalTexturePixelVulkan constructor called";
}

ExternalTexturePixelVulkan::~ExternalTexturePixelVulkan() {
  FT_LOG(Error) << "ExternalTexturePixelVulkan desconstructor called";
  ReleaseImage();
}
...
bool ExternalTexturePixelVulkan::PopulateTexture(size_t width,
  ...
  FT_LOG(Error) << "PopulateTexture called##1 with width: " << width
                << ", height: " << height;
  if (!texture_callback_) {
    FT_LOG(Error) << "texture_callback_ is nullptr";
    return false;
  }
  FT_LOG(Error) << "PopulateTexture called##2 with width: " << width
                << ", height: " << height;
  ...
  FT_LOG(Error) << "PopulateTexture called##3 with width: " << width
                << ", height: " << height;
  ...
  FT_LOG(Error) << "PopulateTexture called##4 with width: " << width
                << ", height: " << height;

  if (!CreateBuffer(width, height)) {
    return false;
  }
  FT_LOG(Error) << "PopulateTexture called##5 with width: " << width
                << ", height: " << height;

  CopyBufferToImage(staging_buffer_, vk_image_, width_, height_,
                    pixel_buffer->buffer);
  FT_LOG(Error) << "PopulateTexture called##6 with width: " << width
                << ", height: " << height;

  ReleaseBuffer();
  FT_LOG(Error) << "PopulateTexture called##7 with width: " << width
                << ", height: " << height;
